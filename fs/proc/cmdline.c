...
static void patch_flag(char *cmd,
		       const char *flag,
		       const char *val)
{
	...
	val_len = strlen(val);
	      ...
	      strcspn(start + flag_len, " ");

	memmove(start + flag_len + val_len,
		end,
		strlen(end) + 1);
...
static int cmdline_proc_open(struct inode *inode,
			     struct file *file)
{
...
static int __init proc_cmdline_init(void)
{
	...
	/*
	 * Patch userspace-visible cmdline flags
	 * for SafetyNet / Play Integrity spoofing
	 */
