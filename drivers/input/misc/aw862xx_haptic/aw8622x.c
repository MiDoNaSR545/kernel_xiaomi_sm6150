				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "lra_calib_flag=%d,lra_calib_data=%d\n",
		   aw8622x->osc_calib_flag,aw8622x->osc_cali_data);
...
static ssize_t aw8622x_lra_ic_vendor_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
...
static ssize_t aw8622x_lra_ic_vendor_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	return -1;
...
static DEVICE_ATTR(lra_ic_vendor, 0644, aw8622x_lra_ic_vendor_show,
		   aw8622x_lra_ic_vendor_store);
	...
	if (aw8622x->state) {
		if (aw8622x->activate_mode ==
			AW8622X_HAPTIC_ACTIVATE_RAM_MODE) {
			    aw8622x_haptic_ram_vbat_compensate(aw8622x, false);//Daniel 20210716 midify start
			    //aw8622x_haptic_play_repeat_seq(aw8622x, true);
			     ...
			     int val)
{
	...
	if (val <= 0) {
		...
		if (aw8622x->activate_mode == AW8622X_HAPTIC_ACTIVATE_RTP_MODE || 
			aw8622x->activate_mode == AW8622X_HAPTIC_ACTIVATE_RAM_LOOP_MODE)
			aw8622x->state = 0;
	}		
	hrtimer_cancel(&aw8622x->timer);
